...
{
  add_submenu_page('edit.php?post_type=product', 'Gift wrap', __('Embrulho de presente', 'neuralab-giftwrap'), 'manage_options', 'gift_wrap_section_menu', 'neuralab_gift_wrap_settings_fields');
}
...
function neuralab_gift_wrap_settings_fields()
{
  ...
  function neuralab_gift_wrap_option()
  {
    if (neuralab_gift_wrap_option_enabled()) {
      ?>
    <div class="gift-wrap">
      <h3><?php _e('Envio para presente', 'neuralab-giftwrap'); ?> </h3>
      <?php

          $gift_wrap_value = (bool) get_option('gift_wrap_value') ? number_format(get_option('gift_wrap_value'), 2, ",", ".") : '0,00';
          ...
          woocommerce_form_field(
            ...
            [
              ...
              // 'label'       => sprintf( __( 'Enviar para Presente (%s)', 'neuralab-giftwrap' ), neuralab_gift_wrap_get_price_formatted() ),
              'label'       => __('Incluir cartão "De / Para" presente neste pedido (R$'.$gift_wrap_value.')', 'neuralab-giftwrap'),
              'placeholder' => '',
            ],
...
add_action('woocommerce_before_order_notes', 'neuralab_gift_wrap_option', 100);
...
function neuralab_gift_wrap_add_fee()
{
  ...
  if (isset($post_data['gift-wrap-check'])) {
    WC()->cart->add_fee(esc_html(__('Incluir cartão "De/Para"', 'neuralab-giftwrap')), neuralab_gift_wrap_tax_price(), neuralab_gift_wrap_tax_enabled());
  }
}
...
{
  $gift_wrap_fee_id = sanitize_title(__('Embrulho para presente', 'neuralab-giftwrap'));
  /** Using WooCommerce function for sanitizing id so it only applies on gift wrap fee */
