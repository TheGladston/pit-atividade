...
} elseif ($price > $min_value)
{
    ...
    if ($installments_price < $min_value)
    {
        ...
        if ($installments_price >= $min_value)
        {
            ...
            $installments_html .= "<p class='price fswp_calc'>" . sprintf(__('<span class="fswp_installment_prefix">%s</span><span class="wc-parc-quantidade"> %sx de </span>', 'wc-parcelas'), $prefix, $installments) . $formatted_installments_price . " <span class='fswp_installment_suffix'>" . $suffix . "</span></p>";
